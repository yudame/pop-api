...
class ItemAddonGroups(Timestampable, Annotatable, models.Model):
    ...
    custom_addon_max_count = models.SmallIntegerField(null=True, blank=True,
        help_text='maxiumum number of addons allowed (eg. 10 salad toppings)')

    custom_per_addon_price = MoneyField(max_digits=8, decimal_places=2, null=True, blank=True, default_currency='THB',
        help_text='default price for each extra addon in this group (eg. toppings are $1 extra each)')
    custom_addon_free_count = models.SmallIntegerField(null=True, blank=True,
        help_text='number of free addons allowed (eg. 1 for salad dressing)')

    # MODEL PROPERTIES
    @property
    def addon_max_count(self):
        return self.custom_addon_max_count or self.addon_group.addon_max_count

    @property
    def per_addon_price(self):
        return self.custom_per_addon_price or self.addon_group.per_addon_price

    @property
    def addon_free_count(self):
        return self.custom_addon_free_count or self.addon_group.addon_free_count
...
class ItemAddonGroupMemberships(Timestampable, Annotatable, models.Model):
    ...
    # production_FnB_serving = models.ForeignKey('production.FnBServing', null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def get_addon_price_display(self):
        if self.custom_addon_price is 0:
            return "Free"
        return self.custom_addon_price or self.addon_group.per_addon_price or "Free"
