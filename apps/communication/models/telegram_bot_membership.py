...
class TelegramBotMembership(Timestampable, models.Model):
    ...
    telegram_user_dict = JSONField(default=dict)
    ...
    def respond_to(self, update: Update, context: CallbackContext):
        ...
        if update.message.video and isinstance(update.message.video, telegram.Video):
            ...
            artist = Artist.objects.filter(user_id=self.user.id).first()
            if not artist:
                return "upload saved, but could not find artist account"

            video = Video.objects.create(
                artist=artist,
                video_upload=upload
            )
            return f"saved as video {video.id}"
